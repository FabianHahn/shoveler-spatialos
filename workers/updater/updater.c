...
int main(int argc, char **argv)
{
	...
	while(!context.disconnected) {
		...
		char command[16] = {0};
		if (scanf("%15[^ \t\n]%*c", command) == EOF) {
			shovelerLogError("Failed to read command.");
			context.disconnected = true;
			break;
		}
		...
		if (strncmp(command, "image", strlen("image")) == 0) {
			...
			if(scanf("%"PRId64"%*c", &resourceEntityId) == EOF) {
				shovelerLogError("Failed to read entity ID.");
				context.disconnected = true;
				break;
			}
			...
			char filename[256] = {0};
			if(scanf("%255[^\n]%*c", filename) == EOF) {
				shovelerLogError("Failed to read filename.");
				context.disconnected = true;
				break;
			}
			...
			int shiftAmount;
			if(scanf("%d%*c", &shiftAmount) == EOF) {
				shovelerLogError("Failed to read shift amount.");
				context.disconnected = true;
				break;
			}
